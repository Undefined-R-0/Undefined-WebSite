<template>
  <!-- Start block -->
  <section class="container mx-auto bg-gray-50">
    <div class="mx-auto px-4 py-8 lg:px-6 lg:py-16">
      <div class="mx-auto max-w-screen-sm text-center">
        <h2 class="mb-4 text-3xl font-extrabold leading-tight tracking-tight text-gray-900">好善乐捐</h2>
        <p class="mb-6 font-light text-gray-500 md:text-lg">
          您的所有捐款将用于支持我们的项目，让更多社会边缘群体受益。促进中国社会的多元化发展，推动民主化改革。
        </p>
        <a
          href="#"
          class="mb-2 mr-2 rounded-lg bg-purple-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-purple-800 focus:outline-none focus:ring-4 focus:ring-purple-300"
        >
          出一份力
        </a>
      </div>
    </div>
  </section>
</template>

<script lang="ts" setup></script>

<style scoped></style>
